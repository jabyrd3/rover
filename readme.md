...
Each robot is processed in the arrays order, i.e.: the engine finishes executing the robots instruction before the next robot begins execution of its instruction. Robots do not interact, besides leaving signals/scents behind when a robot dies from leaving the world bounds.
